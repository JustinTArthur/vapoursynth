...
unsigned sq_interior_byte_dot_sve(const uint8_t *const *rows, uint8_t *dst, unsigned S, unsigned W,
                                  const int32_t *cg, svfloat32_t sc, svfloat32_t bi, svuint32_t smask)
{
    ...
    auto block = [&](unsigned jj) {
        ...
        for (unsigned r = 0; r < N; ++r) {
            ...
            for (unsigned g = 0; g < G; ++g) {
                ...
                const svint8_t w = svreinterpret_s8_s32(svdup_n_s32(cg[r * G + g]));
...
template <unsigned N>
void sq_plane_byte_dot_sve(const void *src, ptrdiff_t ss, void *dst, ptrdiff_t ds,
                           const vs_generic_params &p, unsigned W, unsigned H)
{
    ...
    for (unsigned i = 0; i < H; ++i) {
        ...
        const unsigned aend = sq_interior_byte_dot_sve<N>(rows, d, S, W, cg, sc, bi, smask);
...
VS_SQUARE_DOT_SVE_ENTRY(5x5, 5)
VS_SQUARE_DOT_SVE_ENTRY(7x7, 7)
VS_SQUARE_DOT_SVE_ENTRY(9x9, 9)
VS_SQUARE_DOT_SVE_ENTRY(11x11, 11)
