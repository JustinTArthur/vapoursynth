...
__attribute__((always_inline))
inline void store_row_int(uint8_t *dst, svbool_t pgw, svint32_t acc, int32_t wb,
...
void sme_sq_int32_interior(const uint8_t *const *rows, ptrdiff_t elem_size, uint8_t *dst, ptrdiff_t ds,
                           const int16_t *band, unsigned N, unsigned W, unsigned H,
                           int32_t wb, float div, float fbias, uint32_t satmask, int32_t maxclamp)
{
...
__arm_locally_streaming __arm_new("za")
void sme_sq_int64_interior(const uint8_t *const *rows, uint8_t *dst, ptrdiff_t ds,
                           const int16_t *band, unsigned N, unsigned W, unsigned H,
                           int64_t wb, float div, float fbias, uint32_t satmask, int32_t maxclamp)
{
    const unsigned S = N / 2;
        ...
        for (unsigned cb = S; cb < Wend; cb += 4 * TD) {
            ...
            for (unsigned i = 0; i < vr; ++i) {
                ...
                for (unsigned t = 0; t < 4 && v[t]; ++t) {
                    ...
                    svint64_t acc = svsub_n_s64_x(pt64, read_za64_s64(t, i), wb);
...
{
    const unsigned nd = TR + N - 1;
    const unsigned npair = (nd + 1) / 2;
...
__arm_locally_streaming __arm_new("za")
void sme_sq_half_fmopa_interior(const uint8_t *const *rows, uint8_t *dst, ptrdiff_t ds,
...
__arm_locally_streaming __arm_new("za")
void sme_sq_float_interior(const uint8_t *const *rows, uint8_t *dst, ptrdiff_t ds,
...
__arm_locally_streaming __arm_new("za")
void sme_v_int32_plane(const uint8_t *const *rows, ptrdiff_t elem_size, uint8_t *dst, ptrdiff_t ds,
...
__arm_locally_streaming __arm_new("za")
void sme_v_float_plane(const uint8_t *const *rows, uint8_t *dst, ptrdiff_t ds,
...
__arm_locally_streaming __arm_new("za")
void sme_v_half_fmopa_plane(const uint8_t *const *rows, uint8_t *dst, ptrdiff_t ds,
...
template <class T>
std::vector<const uint8_t *> build_mirror_rows(const void *src, ptrdiff_t ss, unsigned H, unsigned S)
{
    std::vector<const uint8_t *> rows(H + 2 * S);
    for (unsigned t = 0; t < H + 2 * S; ++t)
        rows[t] = static_cast<const uint8_t *>(src) + static_cast<ptrdiff_t>(nc_mirror(static_cast<int>(t) - static_cast<int>(S), static_cast<int>(H))) * ss;
    return rows;
}
                  ...
                  const vs_generic_params &p, unsigned W, unsigned H, unsigned N)
{
    ...
    std::vector<const uint8_t *> rows = build_mirror_rows<T>(src, ss, H, S);
    ...
    if (W > 2 * S) {
        if constexpr (TY == SmeType::Float) {
            ...
            sme_sq_half_fmopa_interior(rows.data(), static_cast<uint8_t *>(dst), ds, band.data(), N, W, H, p.div, p.bias, satmask);
            ...
            int64_t wb = 0;
            for (unsigned i = 0; i < N * N; ++i) wb += static_cast<int64_t>(INT16_MIN) * p.matrix[i];
            sme_sq_int64_interior(rows.data(), static_cast<uint8_t *>(dst), ds, band.data(), N, W, H,
                                  wb, p.div, p.bias, satmask, p.maxval);
        } else {
            unsigned TR = static_cast<unsigned>(svcntsw());
            std::vector<int16_t> band = build_sq_band_i16(p.matrix, N, TR, 2);
            int32_t wb = 0;
            if (TY == SmeType::Word)
                for (unsigned i = 0; i < N * N; ++i) wb += static_cast<int32_t>(INT16_MIN) * p.matrix[i];
            sme_sq_int32_interior(rows.data(), sizeof(T), static_cast<uint8_t *>(dst), ds, band.data(), N, W, H,
                                  wb, p.div, p.bias, satmask, p.maxval);
        }
    }
                 ...
                 const vs_generic_params &p, unsigned W, unsigned H)
{
    ...
    std::vector<const uint8_t *> rows = build_mirror_rows<T>(src, ss, H, support);
    unsigned TR = static_cast<unsigned>(svcntsw());
    ...
    if constexpr (TY == SmeType::Float) {
        ...
        sme_v_half_fmopa_plane(rows.data(), static_cast<uint8_t *>(dst), ds, band.data(), fwidth, W, H, p.div, p.bias, satmask);
    } else {
        std::vector<int16_t> band = build_v_band_i16(p.matrix, fwidth, TR);
        int32_t wb = 0;
        if (TY == SmeType::Word)
            for (unsigned i = 0; i < fwidth; ++i) wb += static_cast<int32_t>(INT16_MIN) * p.matrix[i];
        sme_v_int32_plane(rows.data(), sizeof(T), static_cast<uint8_t *>(dst), ds, band.data(), fwidth, W, H,
                          wb, p.div, p.bias, satmask, p.maxval);
    }
}
...
VS_SME_SQUARE_ENTRY(5x5,   5,  Word,  word)
